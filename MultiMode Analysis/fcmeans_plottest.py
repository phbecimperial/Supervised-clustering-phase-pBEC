...
if __name__ == '__main__':
    ...
    x_tothermal = power_length_analysis.add_loss_rate(ax, 'inter.pkl')
    ...
    # umap plot
    # labels , _ = Meta_classifier.Kmeans_no_CNN(data['Files'][stim_mask], 4)
    ...
    mapp = ax.scatter(cnn_embed[:,0], cnn_embed[:,1], c = (labels + 0.5)/(num_clusters + 0.5), cmap = 'tab20b',s = 5, vmin = 0, vmax = 1)
    ...
    # plt.savefig(rf'C:\Users\Pouis\OneDrive - Imperial College London\Masters\Thesis\Thesis_Plots\UMAP plots\UMAP_Cnn_Fake_Kmeans{num_clusters}.pdf', format = 'pdf')
    plt.show()
    ...
    membership_mat = Meta_classifier.quick_fcmeans(features, num_clusters = num_clusters, m = 7, return_matrix= True)
    ...
    for i, col in enumerate(membership_mat):
        ...
        _,_, plot, stat = power_length_analysis.plot_2d_stat_hist(data['Lengths'][stim_mask],
            ...
            cmap = 'Spectral_r', fig=fig, ax = axes[i], vs = vs, ret_stat=True 
            )
        
        axes[i].set_title(i)
        save_dict[f'cluster {i} probabilities'] = stat[0].tolist()
    ...
    with open('FC_7_clusters_probabilities.json', 'w') as f:
